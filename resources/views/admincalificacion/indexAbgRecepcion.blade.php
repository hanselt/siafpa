...
<div class="container mrg15T row">
    <div class="col-md-12">
        <div class="content-box" style="background-color:#F2F7F8">
            ...
            Expedientes Asignados para Opinión legal: {{ Auth::user()->gen_persona->PERS_varNombres }} {{ Auth::user()->gen_persona->PERS_varApPaterno }} {{ Auth::user()->gen_persona->PERS_varApMaterno }}
            </h3>
                ...
                <table class="table" id="MyTable" name="MyTable">
                    ...
                    @foreach($ListaExpedientes as $Ingreso)
                        <tr id="{{$Ingreso->CONT_varHojaTramite}}BG">
                            ...
                            <td><p style="display: none;" id="{{$Ingreso->CONT_varHojaTramite}}VHD"><p id="{{$Ingreso->CONT_varHojaTramite}}DAT" class="toExport"></p></td>
                            ...
                            <td>
                                <p style="display: none;" class="toExport">Recepción</p>      
                                <a href="recepcionarExpAbg/{{$Ingreso->CONT_varHojaTramite}}" alt="Asignar" title="Recepcionar Exp."><i class="fa fa-check-circle" aria-hidden="true"></i></a>
                            </td>
                            <script type="text/javascript">
                                var FechaAsignacion=new Date("{{$Ingreso->OPIN_datFechaEmisionAbg}}");
                                var FechaIngresoTD=new Date("{{$Ingreso->CONT_datFechaIngresoTD}}");
                                <!---->
                                var CadenaFecha=cadenaDate(FechaAsignacion);
                                <!---->
                                var tdFecha=document.getElementById('{{$Ingreso->CONT_varHojaTramite}}VHD').innerHTML=listaFecha(FechaAsignacion);
                                var tdFecha=document.getElementById('{{$Ingreso->CONT_varHojaTramite}}DAT').innerHTML=CadenaFecha;
                                var FechaActual=new Date();
                                FechaAsignacion=new Date("{{$Ingreso->OPIN_datFechaEmisionAbg}}");
