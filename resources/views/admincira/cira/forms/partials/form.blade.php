...
<div class="row">
    ...
    <div class="col-xs-12 col-md-3">
        <div class="form-group">
            {{ Form::label('CONT_datFechaRecepcionCIRA', 'Fecha de Ingreso en Certificaciones', ['class' => 'label_form']) }}
            {{ Form::date('CONT_datFechaRecepcionCIRA', isset($monumento)?$monumento->CONT_datFechaRecepcionCIRA:\Carbon\Carbon::now(), array('class' => 'form-control','readonly')) }}

            @if($errors->has('CONT_datFechaRecepcionCIRA'))
                <div class="red-text text-darken-2"><i class="fa fa-exclamation-triangle"
                                                       aria-hidden="true"></i> {{$errors->first('CONT_datFechaRecepcionCIRA')}}</div>
            @endif
        </div>
    </div>
