...
@section('title', 'Análisis de Metales')

@section('breadcrumb')
  <li class="active">Análisis de Metales</li>
@endsection
  ...
  <div v-show="esProcesoListado">
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover table-condensed">
        ...
        <tbody>
          <tr v-for="fichaAux in analisisMetales">
            <td>@{{ fichaAux.nro_ficha }}</td>
            <td>@{{ fichaAux.codigo }}</td>
            <td>@{{ fichaAux.gestion_documentaria }}</td>
            <td>
              <div class="btn-group" role="group">
                @can('editar_ficha_metales')
                <btn v-tooltip="'Editar'" type="success" v-on:click="editarAnalisisMetales(fichaAux)"><span class="glyphicon glyphicon-edit"></span></btn>
                @endcan
                @can('eliminar_ficha_metales')
                <btn v-tooltip="'Eliminar'" type="danger" v-on:click="eliminarAnalisisMetales(fichaAux.id)"><span class="glyphicon glyphicon-trash"></span></btn>
                @endcan
                @can('ver_ficha_metales_reporte')
...
<div v-show="!esProcesoListado">
  <dfq-analisis-metales :es-edicion="true" v-model="analisis" v-on:volver="mostrarListado" v-on:actualizar="listarAnalisisMetales(pagination.current_page)" v-on:mensaje="mostrarMensaje"></dfq-analisis-metales>
</div>
@endsection
...
<script>
  let app = new Vue({
    ...
    created: function () {
      this.listarAnalisisMetales(this.pagination.current_page)
    },
    data: {
      abrirModal: false,
      alertarError: false,
    ...
    methods: {
      buscar: function (texto, columna) {
        ...
        this.listarAnalisisMetales(1)
      },
      cambiarPagina: function (page) {
        this.pagination.current_page = page;
        this.listarAnalisisMetales(page);
      },
      editarAnalisisMetales: function (analisisEditar) {
        this.esProcesoListado = false
        this.analisis = jQuery.extend(true, {}, analisisEditar)
        this.titulo = 'Editar Análisis de Metales'
      },
      eliminarAnalisisMetales: function (idFicha) {
      },
      enlaceReporte: function (idFicha) {
        return '/dfq/fichas/analisis_metales/' + idFicha + '/reporte'
      },
      listarAnalisisMetales: function (page) {
        let vm = this
        let uri = '/dfq/fichas/analisis_metales/listar?page=' + page + '&columna=' + vm.busqueda.columna + '&texto=' + vm.busqueda.texto
        axios.get(uri)
        .then(function (response) {
          vm.analisisMetales = response.data.data.data
      ...
      mostrarListado: function () {
        ...
        this.titulo = 'Análisis de Metales'
      },
