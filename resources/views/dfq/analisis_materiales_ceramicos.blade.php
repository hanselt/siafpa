...
@section('title', 'Análisis de Materiales Cerámicos')

@section('breadcrumb')
  <li class="active">Análisis de Materiales Cerámicos</li>
@endsection
  ...
  <div v-show="esProcesoListado">
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover table-condensed">
        ...
        <tbody>
          <tr v-for="fichaAux in analisisCeramicos">
            <td>@{{ fichaAux.nro_ficha }}</td>
            <td>@{{ fichaAux.codigo }}</td>
            <td>@{{ fichaAux.gestion_documentaria }}</td>
            <td>
              <div class="btn-group" role="group">
                @can('editar_analisis_materiales_ceramicos')
                <btn v-tooltip="'Editar'" type="success" v-on:click="editarAnalisisCeramicos(fichaAux)"><span class="glyphicon glyphicon-edit"></span></btn>
                @endcan
                @can('eliminar_analisis_materiales_ceramicos')
                <btn v-tooltip="'Eliminar'" type="danger" v-on:click="eliminarAnalisisCeramicos(fichaAux.id)"><span class="glyphicon glyphicon-trash"></span></btn>
                @endcan
                @can('ver_analisis_materiales_ceramicos_reporte')
...
<div v-show="!esProcesoListado">
  <dfq-analisis-materiales-ceramicos :es-edicion="true" v-model="analisis" v-on:volver="mostrarListado" v-on:actualizar="listarAnalisisCeramicos(pagination.current_page)" v-on:mensaje="mostrarMensaje"></dfq-analisis-materiales-ceramicos>
</div>
@endsection
...
<script>
  let app = new Vue({
    ...
    created: function () {
      this.listarAnalisisCeramicos(this.pagination.current_page)
    },
    data: {
      abrirModal: false,
      alertarError: false,
      ...
      titulo: 'Análisis de Materiales Cerámicos',
    },
    methods: {
      buscar: function (texto, columna) {
        ...
        this.listarAnalisisCeramicos(1)
      },
      cambiarPagina: function (page) {
        this.pagination.current_page = page;
        this.listarAnalisisCeramicos(page);
      },
      editarAnalisisCeramicos: function (analisisEditar) {
        this.esProcesoListado = false
        this.analisis = jQuery.extend(true, {}, analisisEditar)
        this.titulo = 'Editar Análisis de Materiales Cerámicos'
      },
      eliminarAnalisisCeramicos: function (idFicha) {
      },
      enlaceReporte: function (idFicha) {
        return '/dfq/fichas/analisis_materiales_ceramicos/' + idFicha + '/reporte'
      },
      listarAnalisisCeramicos: function (page) {
        let vm = this
        let uri = '/dfq/fichas/analisis_materiales_ceramicos/listar?page=' + page + '&columna=' + vm.busqueda.columna + '&texto=' + vm.busqueda.texto
        axios.get(uri)
        .then(function (response) {
          vm.analisisCeramicos = response.data.data.data
      ...
      mostrarListado: function () {
        ...
        this.titulo = 'Análisis de Materiales Cerámicos'
      },
