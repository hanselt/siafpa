...
@section('title', 'Análisis de Aguas')

@section('breadcrumb')
  <li class="active">Análisis de Aguas</li>
@endsection
  ...
  <div v-show="esProcesoListado">
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover table-condensed">
        ...
        <tbody>
          <tr v-for="fichaAux in analisisAguas">
            <td>@{{ fichaAux.nro_ficha }}</td>
            <td>@{{ fichaAux.codigo }}</td>
            <td>@{{ fichaAux.gestion_documentaria }}</td>
            <td>
              <div class="btn-group" role="group">
                @can('editar_analisis_aguas')
                <btn v-tooltip="'Editar'" type="success" v-on:click="editarAnalisisAguas(fichaAux)"><span class="glyphicon glyphicon-edit"></span></btn>
                @endcan
                @can('eliminar_analisis_aguas')
                <btn v-tooltip="'Eliminar'" type="danger" v-on:click="eliminarAnalisisAguas(fichaAux.id)"><span class="glyphicon glyphicon-trash"></span></btn>
                @endcan
...
<div v-show="!esProcesoListado">
  <dfq-analisis-aguas :es-edicion="true" v-model="analisis" v-on:volver="mostrarListado" v-on:actualizar="listarAnalisisAguas(pagination.current_page)" v-on:mensaje="mostrarMensaje"></dfq-analisis-aguas>
</div>
@endsection
...
<script>
  let app = new Vue({
    ...
    created: function () {
      this.listarAnalisisAguas(this.pagination.current_page)
    },
    data: {
      abrirModal: false,
      alertarError: false,
    ...
    methods: {
      buscar: function (texto, columna) {
        ...
        this.listarAnalisisAguas(1)
      },
      cambiarPagina: function (page) {
        this.pagination.current_page = page;
        this.listarAnalisisAguas(page);
      },
      editarAnalisisAguas: function (analisisEditar) {
        this.esProcesoListado = false
        this.analisis = jQuery.extend(true, {}, analisisEditar)
        this.titulo = 'Editar Análisis de Aguas'
      },
      eliminarAnalisisAguas: function (idFicha) {
      },
      enlaceReporte: function (idFicha) {
        return '/dfq/fichas/analisis_aguas/' + idFicha + '/reporte'
      },
      listarAnalisisAguas: function (page) {
        let vm = this
        let uri = '/dfq/fichas/analisis_aguas/listar?page=' + page + '&columna=' + vm.busqueda.columna + '&texto=' + vm.busqueda.texto
        axios.get(uri)
        .then(function (response) {
          vm.analisisAguas = response.data.data.data
