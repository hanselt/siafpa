      ...
      <table class="table table-striped table-hover table-condensed" style="table-layout: fixed;">
        ...
        <tbody>
          <tr v-for="(rol, index) in roles">
            <td>@{{ index + 1 }}</td>
            <td>@{{ _.startCase(rol.name) }}</td>
            <td>
              <div class="btn-group" role="group">
                @can('editar_roles')
                <btn v-tooltip="'Editar'" type="success" v-on:click="editarRol(rol)"><span class="glyphicon glyphicon-edit"></span></btn>
                @endcan
              </div>
            </td>
          </tr>
...
<div v-show="!esProcesoListado">
  <rol v-model="rol" :es-edicion="true" v-on:mensaje="mostrarMensaje" v-on:volver="volverAlListado"></rol>
</div>
@endsection
...
<script>
  const app = new Vue({
    ...
    created: function () {
      this.listarRoles(this.pagination.current_page);
    },
    computed: {
    ...
    methods: {
      ...
      editarRol: function (rolEditar) {
        let vm = this
        vm.titulo = 'Editar Rol'
        vm.rol = jQuery.extend(true, {}, rolEditar)
        vm.esProcesoListado = false
      },
      listarRoles: function (page) {
        let vm = this
        let uri = '/listar/roles?page=' + page + '&columna=' + vm.busqueda.columna + '&texto=' + vm.busqueda.texto
        axios.get(uri)
        .then(function (response) {
          vm.roles = response.data.data.data
